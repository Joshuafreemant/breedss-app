...
export default function CatsSocial() {
    const [posts, setPosts] = useState([]);
    const [blobUrls, setBlobUrls] = useState([]);


    const getPost = async () => {
        await axios.get("/CatPosts").then((response) => {
            ...
            <section className="section post__section-parent">
                {!posts.length ? (
                    <div className="container no-post">
                        ...
                        <p>Be the first to create by clicking the CREATE button and select the Cats category</p>
