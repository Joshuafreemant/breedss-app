...
export default function OtherSocial() {
    ...
    const getPost = async () => {
        await axios.get("/otherPosts").then((response) => {
            ...
            <section className="section post__section-parent">
                {!posts.length ? (
                    <div className="container no-post">
                        ...
                        <p> Your Pet Category is not listed? No Worries, be the first
                        to create a post on the Other-Pets Category by clicking 
                        the CREATE button and select the Others category</p>
